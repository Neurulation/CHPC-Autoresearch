"""Spiking GRU — Phase B of the ANP SNN project.

Hybrid spiking-recurrent model for sequential MNIST. Identical to SNN-LSTM
but replaces nn.LSTM with nn.GRU. The input stage uses LIF neurons (rate
coding: T=25 Bernoulli timesteps per row) to produce spike-count features.

Architecture
------------
Input  : (N, 1, 28, 28) MNIST → viewed as 28 rows of 28 pixels
Per row: Bernoulli rate-encode (T=25) → LIF integration → spike counts (28,)
GRU    : 2-layer stacked, input_size=28, hidden_size=256
...
class SNNGRUModel(nn.Module):
    """Spiking GRU for sequential MNIST (row-level, T_seq=28).
    ...
    Args:
        ...
        dropout:     GRU inter-layer dropout (ignored if num_layers == 1).
